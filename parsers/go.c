...
static void parseConstTypeVar (tokenInfo *const token, goKind kind, int scope)
{
	...
	do
	{
			...
			if (isType (token, TOKEN_IDENTIFIER))
			{
				if (kind == GOTAG_TYPE)
				{
					...
					if (isKeyword (token, KEYWORD_struct))
						scope = makeTag (typeToken, GOTAG_STRUCT, scope, NULL);
					else if (isKeyword (token, KEYWORD_interface))
						scope = makeTag (typeToken, GOTAG_INTERFACE, scope, NULL);
					else
						scope = makeTag (typeToken, kind, scope, NULL);
					break;
				}
				else
		...
		{
			if (isKeyword (token, KEYWORD_struct))
				parseStructMembers (token, scope);
			else
				skipType (token);
